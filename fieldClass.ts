...
export class Field {
  ...
  testField() {
    ...
    const moveInputTest = (position:number[], input:string) => {
      switch (input) {
        case "l":
          if (position[0] !== 0) return [position[0] -= 1, position[1]];
          break;
        case "r":
    ...
    function moveResult(position: number[], field: string[][]) {
